...
public class IosSimulatorDeviceFinder implements DeviceFinder<IosSimulator> {
    ...
    private final String XCRUN_SIMCTL_SIMULATOR_INFORMATION_DEVICE_LIST_JSON =
            "xcrun simctl list devices --json devices";
    private final String XCRUN_XCDEVICE_SIMULATOR_INFORMATION_DEVICE_LIST_JSON =
            "xcrun xcdevice list";

    @Override
    public DeviceInfoModel<IosSimulator> findDevices(String localPath) throws IOException {
        DeviceInfoModel<IosSimulator> deviceInfoModel =
        ...
        if (deviceInfoModel == null
            ...
            try {
                throw new DeviceNotFoundException("iOS Simulated Device Not Found !!!");
    ...
    public Map<String, Object> readDeviceInfo(String localPath) throws IOException {
        ...
        Process simctlProcess =
                ProcessHelper.runTimeExec(
                        String.format("%s%s", localPath, XCRUN_SIMCTL_SIMULATOR_INFORMATION_DEVICE_LIST_JSON));
        BufferedReader simctlReader =
                new BufferedReader(new InputStreamReader(simctlProcess.getInputStream()));
        ...
        Process xcdeviceProcess =
                ProcessHelper.runTimeExec(
                        String.format(
                                "%s%s", localPath, XCRUN_XCDEVICE_SIMULATOR_INFORMATION_DEVICE_LIST_JSON));
        BufferedReader xcdeviceReader =
                new BufferedReader(new InputStreamReader(xcdeviceProcess.getInputStream()));
        ...
        JsonObject devicesListJson =
                new Gson()
                        .fromJson(String.valueOf(deviceListStringBuilder), JsonObject.class)
                        .getAsJsonObject("devices");
        JsonArray devicesDetailInfoJson =
                new Gson().fromJson(String.valueOf(deviceDetailInfoStringBuilder), JsonArray.class);
                ...
                if (currentKey.get("state").toString().contains("Booted")) {
                    ...
                    for (JsonElement currentElement : devicesDetailInfoJson) {
                        ...
                        if (currentElementValue
                                .get("identifier")
                                .toString()
                                .contains(currentKey.get("udid").toString())) {
